$(window).on("load", function () {
  ...
  var horizontalChart = new Chart(horizontalChartctx, horizontalChartconfig);
	...
	$.ajax({
		url: "leadDashboardPaiChart.php",
		type:"post",
		catch:false,
		success: function(result)
		{
			var res = result.split("~");
			totalAllocation = res[0];
			pendingLead = res[1];
			processLead = res[2];
			positiveLead = res[3];
			negativeLead = res[4];
	...
	var piechartData = {
		labels: ["Total Allocation", "Pending Lead", "Process Lead", "Positive Lead", "Negative Lead", "Converted Lead"],
		datasets:
		[{
			label: "My First dataset",
			data: [totalAllocation, pendingLead, processLead, positiveLead, negativeLead, convertLead],
			backgroundColor: [$primary, $black, $danger, $warning, $label_color, $success],
		}]
	};
	var pieChartconfig = {
		type: 'pie',
